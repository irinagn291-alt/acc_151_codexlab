...
enum CodexLabOpenLibraryError: Error, Sendable {
    case notFound
    case invalidISBN
}

struct CodexLabOpenLibraryClient: CodexLabOpenLibraryAPIProtocol {
    func fetchBook(isbn: String) async throws -> CodexLabOpenLibraryBook {
        let normalized = normalizeISBN(isbn)
        guard !normalized.isEmpty else { throw CodexLabOpenLibraryError.invalidISBN }
        let resolved = try await resolveEdition(isbn: normalized)
        return CodexLabOpenLibraryBook(
    ...
    func fetchRecommendations(subjects: [String], limit: Int) async throws -> [CodexLabBookRecommendation] {
        ...
        for subject in subjects {
            ...
            guard let (data, response) = try? await URLSession.shared.data(from: url),
                  ...
                  let payload = try? JSONDecoder().decode(CodexLabOpenLibrarySearchResponse.self, from: data) else { continue }
            ...
            for doc in payload.docs.sorted(by: { ($0.cover_i != nil) && ($1.cover_i == nil) }) {
                ...
                results.append(CodexLabBookRecommendation(
                    id: dedupeKey,
                    isbn: primaryISBN,
    ...
    private func fetchEditionJSON(isbn: String) async throws -> EditionJSON {
        guard let url = URL(string: "https://openlibrary.org/isbn/\(isbn).json") else {
            throw CodexLabOpenLibraryError.invalidISBN
        }
...
struct CodexLabOpenLibrarySearchResponse: Decodable, Sendable {
    let docs: [CodexLabOpenLibrarySearchDoc]
}

struct CodexLabOpenLibrarySearchDoc: Decodable, Sendable {
    let key: String?
    let title: String?
